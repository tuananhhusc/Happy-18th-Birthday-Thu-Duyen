  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      const interval = setInterval(() => {
        ...
        toast.custom((t) => (
          <div className={`${notif.color} w-full max-w-sm border-4 border-black p-4 shadow-[6px_6px_0_0_#000] flex gap-4 items-start animate-in slide-in-from-right duration-300`}>
            <div className="text-3xl bg-white border-2 border-black rounded-full p-2 w-12 h-12 flex items-center justify-center">
              {notif.icon}
            </div>
            <div className="flex-1">
              <h4 className="font-black text-lg uppercase mb-1">{notif.title}</h4>
              <p className="font-bold text-sm leading-tight text-gray-800">{notif.msg}</p>
            </div>
            <button 
              onClick={() => toast.dismiss(t)} 
              className="text-black font-black text-xl hover:scale-125 transition-transform"
            >
              ×
            </button>
          </div>
